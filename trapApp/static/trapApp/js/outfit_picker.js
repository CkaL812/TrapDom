document.addEventListener('DOMContentLoaded', function () {
    ...
    let   offset     = 0;          // перший видимий індекс
    let   activeIdx  = -1;
    ...
    btnUp.addEventListener('click', () => {
        if (offset > 0) { offset--; renderWheel(); }
    });

    btnDown.addEventListener('click', () => {
        if (offset + VISIBLE < TOTAL) { offset++; renderWheel(); }
    });
    ...
    function checkReady() {
        ...
        const allSelect = genderSelect.value && seasonSelect.value && styleSelect.value;
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                resultText.textContent = data.message || 'Образ успішно підібрано!';
            }, 400);
        } catch (err) {
            clearInterval(tick);
            progress.style.width = '0%';
